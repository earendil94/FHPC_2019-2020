...
int main(int argc, char **argv){
    ...
    //Particles declaration and read
    particle *pars;
    ...
    do{
        ...
        FILE *ofp;
        char ofpFilename[30];
        snprintf(ofpFilename, sizeof(ofpFilename), "particles_%d.ic", n_it);
        ...
        for(int register i = 0; i < n; i++){     
            for( int register k = 0; k < n; k++){
                ...
                if( k != i){
                    ...
                    // if((par[i].p[1] == par[k].p[1]) || (par[i].p[2] == par[k].p[2])){
                    // printf("Exception found at %i and %d\n", i, k);
                    // exit(-1);
                    // }
